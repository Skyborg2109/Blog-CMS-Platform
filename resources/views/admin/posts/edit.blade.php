...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Post</h1>
</div>

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-w-4xl mx-auto">
    <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8">
        @csrf
        @method('PUT')
                ...
                <div class="bg-gray-50/50 p-5 rounded-xl border border-gray-100">
                    ...
                    @if($post->image)
                        <div class="mb-3 relative rounded-lg overflow-hidden border border-gray-200 group">
                            <img src="{{ Storage::url($post->image) }}" class="w-full h-32 object-cover" alt="Current cover">
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-gray-400 mt-2">Leave empty to keep existing image</p>
                ...
                <div class="bg-gray-50/50 p-5 rounded-xl border border-gray-100">
                    ...
                    <div class="space-y-2 max-h-48 overflow-y-auto w-full pr-2">
                        ...
                        <label class="flex items-center space-x-3 p-2 hover:bg-white rounded-lg transition-colors cursor-pointer border border-transparent hover:border-gray-100">
                            <input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $postTags)) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 bg-gray-100 border-gray-300 rounded focus:ring-indigo-500 focus:ring-2">
