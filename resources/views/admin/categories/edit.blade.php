...
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.categories.index') }}" class="text-sm text-gray-500 hover:text-indigo-600 font-medium flex items-center transition mb-2">
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-w-2xl">
    <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="p-6 md:p-8">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <input type="text" disabled value="{{ $category->slug }}" class="w-full px-4 py-2.5 border border-gray-200 bg-gray-50 text-gray-500 rounded-xl text-sm">
        </div>
